...
def find_equinox_launcher(jdtls_base_directory):
    ...
    # Primero buscar la versión con patrón de versión
    launchers = list(plugins_dir.glob('org.eclipse.equinox.launcher_*.jar'))
    if launchers:
        return str(launchers[0])  # Tomar el primer match
    
    # Si no se encuentra, buscar el nombre sin versión
    launcher_no_version = plugins_dir / "org.eclipse.equinox.launcher.jar"
    if launcher_no_version.exists():
        return str(launcher_no_version)
